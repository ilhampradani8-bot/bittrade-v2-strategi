...
def evaluate_subperiod(df_sub, combinations, step_a, step_b, min_a, min_b):
    ...
    for interval_str, window_size in combinations:
        ...
        # Fast Rolling OLS
        endog = df_resampled['log_a']
        exog = sm.add_constant(df_resampled['log_b'])
        ...
        df_resampled['beta'] = params['log_b']
        
        df_resampled['residual'] = df_resampled['log_a'] - (df_resampled['alpha'] + df_resampled['beta'] * df_resampled['log_b'])
        ...
        # Iterate only over valid rows
        for idx, row in df_valid.iterrows():
            beta = row['beta']
            alpha = row['alpha']
            std_err = row['std_err']
            
            curr_spread = row['log_a'] - beta * row['log_b']
            ...
            if not in_pos:
                ...
                if hit_exit or hit_sl:
                    curr_price_a = row['price_a']
                    curr_price_b = row['price_b']
        ...
        median_se = df_valid['std_err'].median() * 100
