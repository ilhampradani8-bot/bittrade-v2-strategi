...
for i in range(window, len(df)):
    ...
    if not in_pos:
        if abs(z) > z_entry and std_err > HURDLE:
            w_btc = (abs(beta) * 130) / (1 + abs(beta))
            if w_btc < 62.0: continue
            in_pos = True
            pos_dir = -np.sign(z)
            entry_spread = curr_spread
            entry_beta = beta
            print(f"ENTRY: beta={beta:.2f}, std_err={std_err:.4f}, z={z:.2f}, spread={curr_spread:.4f}")
    else:
        if (pos_dir == 1 and z > -z_exit) or (pos_dir == -1 and z < z_exit):
            ...
            print(f"EXIT: z={z:.2f}, spread={curr_spread:.4f}, gross={gross:.4f}, gross_pct={gross_pct:.4f}")
            break
