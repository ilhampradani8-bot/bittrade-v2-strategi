...
pub async fn load_arb_positions_from_db(state: &AppState) -> Result<(), sqlx::Error> {
    ...
    let lifetime_funding: f64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(payment_amount), 0.0) FROM alt_funding_log"
    )
    .fetch_one(&state.db)
    .await
    .unwrap_or(0.0);
    ...
    println!(
        "[CRASH RECOVERY] Loaded {} posisi arb dari DB. Active positions funding: ${:.4}. Lifetime funding collected: ${:.4}",
        positions.len(),
        total_collected,
        lifetime_funding
    );

    *state.total_funding_collected.write().await = lifetime_funding;
    Ok(())
}
