...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ────────── Hitung saldo awal ──────────
    let default_capital: f64 = std::env::var("INITIAL_BALANCE")
        .ok()
    ...
    let available_balance = (initial_balance - (deployed_capital * 1.1)).max(0.0);
    println!("[INIT] Saldo tersedia: ${:.2} (Total: ${:.2}, Deployed: ${:.2})",
        available_balance, initial_balance, deployed_capital * 1.1);
...
async fn get_history(Extension(state): Extension<AppState>) -> impl IntoResponse {
    let rows = sqlx::query_as::<_, TradeHistory>(
        "SELECT id, action, symbol, price, amount, timestamp, status, notes
