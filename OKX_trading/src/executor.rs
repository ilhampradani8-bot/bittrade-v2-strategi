...
pub async fn execute_trade(decision: &Decision, price: f64, state: &AppState) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    match decision {
        Decision::Buy(amount, reason) => {
            ...
            if *sim_bal >= total_cost {
                ...
                *state.high_water_mark.write().await = price;
                let _ = sqlx::query("INSERT INTO okx_active_positions (buy_price, high_water_mark, amount) VALUES ($1, $2, $3)")
                    .bind(price)
                    .bind(price)
                    .bind(*amount)
                    .execute(&state.db)
                ...
                sqlx::query(
                    "INSERT INTO okx_trading_history (action, price, amount, status, notes) VALUES ($1, $2, $3, $4, $5)"
                )
                .bind("BUY")
                .bind(price)
                .bind(*amount)
        ...
        Decision::Sell(amount, reason) => {
            // 1. Query weighted average entry price dari okx_active_positions BEFORE deleting them!
            ...
            if *btc_bal >= *amount {
                ...
                *state.high_water_mark.write().await = 0.0;
                ...
                // 2. Bersihkan posisi aktif karena kita telah mengekstrak metriknya
                let _ = sqlx::query("DELETE FROM okx_active_positions").execute(&state.db).await;

                // 3. Hitung true Net P&L berdasarkan weighted average entry cost
