...
pub async fn open_arb_position(
    ...
    let total_cost = position_size_usdt + fee_spot + fee_futures;
...
pub async fn close_arb_position(
    ...
    // Biaya penutupan
    let fee_close = pos_info.position_size_usdt * 0.001   // spot sell fee
                  + pos_info.position_size_usdt * 0.0004;  // futures close fee
    ...
    {
        let mut bal = state.simulated_balance.write().await;
        *bal += pos_info.position_size_usdt + total_pnl;
    }
